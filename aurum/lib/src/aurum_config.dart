...
/// [AurumConfig.current].
///
/// To override the current configuration, use [AurumConfig.runWithConfig].
/// The function passed to this method will receive the provided config
/// whenever [AurumConfig.current] is called. See
/// [AurumConfig.runWithConfig] for more details.
///
...
class AurumConfig extends Equatable {
  ...
  factory AurumConfig.current() {
    final zoneValue = Zone.current[currentConfigKey] as AurumConfig?;
    return zoneValue ?? const AurumConfig();
  }

  /// The [Symbol] used to look up and assign the default [AurumConfig] in
  /// a [Zone].
  ///
  ...
  ///
  /// This method is commonly used to override the default [AurumConfig]
  ...
  /// directory, which can be used to override the default [AurumConfig]
  ...
  /// ```dart
  /// void main() {
  ///   AurumConfig.runWithConfig(
  ///     // This example uses AurumConfig.copyWith to override the
  ///     // forceUpdateGoldenFiles and theme properties.
  ///     config: AurumConfig.current().copyWith(
  ///       forceUpdateGoldenFiles: true,
  ///       theme: ThemeData.dark(),
  ///     ),
  ///     run: () {
  ///       test('some test', () {
  ///         expect(AurumConfig.current().forceUpdateGoldenFiles, isTrue);
  ///         expect(AurumConfig.current().theme, equals(ThemeData.dark()));
  ///       });
  ///     },
  ///   );
  /// }
  ...
  /// void main() {
  ///   AurumConfig.runWithConfig(
  ///     // This example uses AurumConfig.merge to override nested settings
  ...
  ///     config: AurumConfig.current().merge(
  ///       AurumConfig(
  ///         platformGoldensConfig: PlatformGoldensConfig(
  ///           theme: ThemeData.dark(),
  ///         ),
  ...
  ///       test('some test', () {
  ///         expect(
  ///           AurumConfig.current().platformGoldensConfig.theme,
  ...
  /// Creates a copy and merges this [AurumConfig] with the given config,
  ...
  /// details.
  AurumConfig merge(AurumConfig? other) {
    return copyWith(
...
/// [AurumConfig]'s theme will be used. If that is also `null`, the default
...
abstract class GoldensConfig extends Equatable {
  ...
  /// The [ThemeData] to use when generating golden tests.
  ///
  /// If no [ThemeData] is provided, the enclosing [AurumConfig]'s theme
